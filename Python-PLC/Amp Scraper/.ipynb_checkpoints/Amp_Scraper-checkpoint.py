...
Conversion_Rate = 30.188679 #Convert amp forward power from percent to Watts
...
while True:
    ...
    #Amp_Response = os.popen("curl -u admin:admin http://169.254.1.1/status.xml")
    
    Amp_Response = os.popen("curl -u admin:admin http://10.50.0.21/status.xml")
    ...
    Values[-1] = str(float(Values[8]) * Conversion_Rate) #Adding the converted
                                                        # FWD Power value
