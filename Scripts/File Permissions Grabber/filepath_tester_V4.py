...
for number,path in enumerate(paths):
    if len(path) == 3:
        cleaned_paths.append('N:\\')
        continue
    cleaned_paths.append("N:\\" + path[6:])
...
for number, user in enumerate(users):
    cleaned_users.append(re.sub("\\\\",'',user))
...
workbook = xlsxwriter.Workbook("{}_Permissions_Audit.xlsx".format(drive_and_date))
worksheet = workbook.add_worksheet()


data_list = clean_data.values.tolist()
for number, i in enumerate(data_list):
    for num, j in enumerate(i):
        worksheet.write(number,num,data_list[number][num])

print("Formatting Excel document")
worksheet.add_table('A1:{}{}'.format(numtotitle(len(data_list[0])),len(data_list)), {'style': 'Table Style Light 15',\
                                            'columns': [{'header': 'Path'},\
                                           {'header': 'Domain'},\
                                           {'header': 'Users'},\
                                           {'header': 'Permissions'},\
                                           {'header': 'Permissions Expl.'},\
                                           {'header': 'Inherited'},\
                                           {'header': 'Root'}]})

worksheet.freeze_panes(1,0)
#Autowidth by tracking max value and then setting it that way?

print("Saving {}_Permissions_Audit.xlsx".format(drive_and_date))
workbook.close()
print("Finished saving {}_Permissions_Audit.xlsx".format(drive_and_date))

input("All Done:::Please hit enter to close this command prompt")
