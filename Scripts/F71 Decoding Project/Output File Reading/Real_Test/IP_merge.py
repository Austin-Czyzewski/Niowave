...
def convert_to_1kgNU(NU, timetable) :
    ...
    Time = NU['Time ({})'.format(timetable)]
    NU = NU.drop(columns = 'Time ({})'.format(timetable))
...
def U_237_adder(NU,LEU, timetable) :
    ...
    try :
        ...
        exit()
    Time = pd.Series.to_numpy(NU['Time ({})'.format(timetable)],dtype = 'float')
...
#         if time_units == 'Years' and (Time[time_values] == 4.006471 or Time[time_values] ==4.004729):
        if timetable == 'Years' and (Time[time_values] == 4.006471 or Time[time_values] ==4.004729):
            #print('\nin here\n')
            dU_237 = 0.0
...
def output_reader(filename, NOI, timetable = 'days'):
    """
    ...
    All_FP.insert(loc = 0, column = 'Time ({})'.format(timetable),value = All_FP.index)
    All_AC.insert(loc = 0, column = 'Time ({})'.format(timetable),value = All_AC.index)
...
def dataframe_merger(List_of_DataFrames, timetable):
    """
    ...
    if Portion_of_core == 1:
        List_of_DataFrames[0] = List_of_DataFrames[0].drop(columns = ["Time ({})".format(timetable)])
...
#         print('Wow, that took {:.1f} seconds'.format(time.time()-time1))
    if Portion_of_core == 2:
        Total_Activity = convert_to_1kgNU(List_of_DataFrames[1], timetable = timetable)
...
#Run it
###############################################################################
LEU = output_reader(files[file_number_LEU], plotting_type, timetable = time_units.lower())
NU = output_reader(files[file_number_NU], plotting_type, timetable = time_units.lower())
dfs = dataframe_merger([LEU, NU], timetable = time_units.lower())
The_Inventory = dfs
