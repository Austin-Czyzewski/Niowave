...
print(Tunnel, IP, IF_Channel, Trigger_Channel , Trigger_Level,\
Read_Start_Voltage, Measurement, Step_size, Pulse_Step_size, \
Max_Threshold, Walk_Threshold, Pulse_Walk_Threshold, \
Wait_after_step, Wait_between_reads, Interlock_Threshold_mv, \
Loops_Debounce, Interlock_Debounce)
...
OS = RM.open_resource('GPIB0::1::INSTR') #Opening the oscilloscope as an object
...
Max_Threshold = 1000000000 #(Hz) Total amount of frequency change before automatically tripping off program
...
Wait_after_step = 0.0100 #Seconds, the time waited after a step is taken, necessary to allow oscope measurements to change
Wait_between_reads = 0.0100 #Seconds, currently not used, supplemented by GUI time between reads
Interlock_Threshold_mv = 50 #mv, this is the amount of deviation before regulation trips off


size = 8 #Size marker used universally between buttons

Loops_Debounce = 2
Interlock_Debounce = 3
Interlock_Debounce_Current = 0
...
def scanning(): #Defining what is happening when scanning our GUI
    ...
    global Interlock_Debounce_Current
            ...
            if read_value > (Error_signal_offset + Pulse_Walk_Threshold): #Checks to see if that value is outside of threshold
                ...
                if Ups > Loops_Debounce: #Effective debounce
                    ...
                    if read_value > (Error_signal_offset + Interlock_Threshold_mv):
                        Interlock_Debounce_Current += 1
                        if Interlock_Debounce_Current >= Interlock_Debounce:
...
def regulation_setpoint_set():
    ...
    if pulsing:
        ...
        Error_signal_offset = GPIB.read_mv(OS, long = Long, measurement = Measurement)
    pass_value(external = True, value = Error_signal_offset)
    
def pass_value(external = False, value = 0):
