...
for run in runs:
    ...
    #############################
    ### To our Optimized point
    #############################
    
    Temp_Data_Array = Full_Data_Set[iterations:]
    
    iteratations += Right_Steps + Left_Steps + Upward_Steps + Downward_Steps + 4
    
    Temp_Dump_1 = Temp_Data_Array[:,4]
    Temp_Dump_2 = Temp_Data_Array[:,5]
    Temp_Dump_Sum = Temp_Dump_1 + Temp_Dump_2
    
    Temp_Horizontal_Percent = Temp_Dump_Sum[:(Right_Steps + 2 + Left_Steps)]/Start_Current*100
    Temp_Vertical_Percent = Temp_Dump_Sum[(Right_Steps + 2 + Left_Steps):]/Start_Current*100
    
    Horizontal_Max_Value = int(np.where(Temp_Horizontal_Percent == max(Temp_Horizontal_Percent)))
    Vertical_Max_Value = int(np.where(Temp_Vertical_Percent == max(Temp_Vertical_Percent)))
    
    Temp_WF6H = Temp_Data_Array[iterations:(iterations + Right_Steps + 2 + Left_Steps),0]
    Temp_WF7H = Temp_Data_Array[iterations:(iterations + Right_Steps + 2 + Left_Steps),1]
    Temp_WF6V = Temp_Data_Array[(iterations + Right_Steps + 2 + Left_Steps):,2]
    Temp_WF7V = Temp_Data_Array[(iterations + Right_Steps + 2 + Left_Steps):,3]
    
    Optimized_WF6H = Temp_WF6H[Horizontal_Max_Value]
    Optimized_WF7H = Temp_WF7H[Horizontal_Max_Value]
    Optimized_WF6V = Temp_WF6V[Vertical_Max_Value]
    Optimized_WF7V = Temp_WF7V[Vertical_Max_Value]

    M.Ramp_Two(Client, WF6H_Tag, WF7H_Tag, Magnet_1_Stop = Optimized_WF6H, Magnet_2_Stop = Optimized_WF7H, Resolution = Read_Steps, sleep_time = .100)
    M.Ramp_Two(Client, WF6V_Tag, WF7V_Tag, Magnet_1_Stop = Optimized_WF6V, Magnet_2_Stop = Optimized_WF7V, Resolution = Read_Steps, sleep_time = .100)
