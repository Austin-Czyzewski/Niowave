#####################
#Run with the following command in command window
...
from bokeh.models import LinearAxis, Range1d, BoxSelectTool, LogAxis
...
p = figure(plot_width=1000, plot_height=400,
           ...
           tools="pan,xwheel_zoom,xbox_zoom,reset",
...
r1 = p.line([Start_Time], [1e-6], color="yellow", line_width=2, y_range_name = "pressures")
...
p.extra_y_ranges = {"temps": DataRange1d(only_visible = True, renderers = [r2,r3]), 
                   "pressures": DataRange1d(only_visible = True,  renderers = [r1], default_span = 1000)}

p.add_layout(LinearAxis(y_range_name="temps", axis_label = "Temps"), 'left')
p.add_layout(LogAxis(y_range_name="pressures", axis_label = "Pressures"), 'left')
...
def update(step):
    ...
    ds1.data['y'].append(random.randint(1,100)*1e-6)
